...
def YetinHairToolWindows():
    if(cmds.window('YetinHairToolWindows',q=True,ex=True)):cmds.deleteUI('YetinHairToolWindows')
    
    cmds.window('YetinHairToolWindows',t=u'cg_YetinHairTools v.0.04', w=wd)
...
def copyAttr(attr, slider, *arg):
    ...
    if isRightList():
        ...
        for folicle in folicles:
            ...
            if attr==2:
                value = cmds.getAttr(hairSys[0] + '.stiffnessScale[0].stiffnessScale_FloatValue')
                print (hairSys[0] + '.stiffnessScale[0].stiffnessScale_FloatValue     ' + str(value))
                cmds.setAttr(folicle[0] + '.stiffness', value)
            else:
